...
export default function Feed({ post, isModal = false }: IFeedProps) {
  ...
  return (
    <>
      {post && (
        <div className="w-full p-3 space-y-3 bg-indigo-100 border shadow-md h-fit rounded-2xl">
          <header className="flex justify-between">
            ...
            >
              ...
              <h1 className="text-lg font-bold cursor-pointer">{user.name}</h1>
            </div>
          ...
          >
            <div
              className={cls(
                ...
                mediaType === "Video" ? "space-y-0 px-4 py-2" : "space-y-2 p-4"
              )}
            >
              <article>
      ...
      {isOpen ? (
        <PostModal isOpen={isOpen} setIsOpen={setIsOpen} postId={post.id} />
      ) : null}
    </>
  );
}
